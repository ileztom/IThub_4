...
@Serializable
data class CommentCreate(val postId: Int, val author: String, val content: String, val date: String? = null)

@Serializable
data class Comment(val id: Int, val postId: Int, val author: String, val content: String, val date: String? = null)

fun Route.registerCommentRoutes(service: CommentService) {
    route("/comments") {
        get { call.respond(service.getAll()) }
        post {
            val payload = try { call.receive<CommentCreate>() } catch (e: Exception) {
        ...
        route("/{id}") {
            get {
                ...
                service.get(id)?.let { call.respond(it) } ?: call.respond(HttpStatusCode.NotFound, mapOf("error" to "Comment not found"))
            }
            put {
